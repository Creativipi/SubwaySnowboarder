...
int cmdGenMoveActPos(int act_id, bool ch_pos, int x, int y, bool ch_flipX, bool flipX, bool ch_flipY, bool flipY) {
    ...
    x = MASK_N_BITS(x, 10); // Ensure x is within 10 bits
    y = MASK_N_BITS(y, 10); // Ensure y is within 10 bits
    return (opcode << 28) | (act_id << 25) | (ch_pos << 24) | (x << 14) | (y << 4) | (ch_flipX << 3) | (ch_flipY << 2) | (flipX << 1) | (flipY << 0);
}
...
int cmdGenSetActTile(int act_id, bool ch_tile, int tile_id, bool ch_flipX, bool flipX, bool ch_flipY, bool flipY) {
    ...
    return (opcode << 28) | (act_id << 25) | (ch_tile << 24) | (tile_id << 20) | (ch_flipX << 3) | (ch_flipY << 2) | (flipX << 1) | (flipY << 0);
}
...
int cmdGenSetBackPixTile(int tile_id, int colorCode, int x, int y) {
    int opcode = 0x6;
    tile_id = MASK_N_BITS(tile_id, 5); // Ensure tile_id is within 4 bits
    x = MASK_N_BITS(x, 3); // Ensure x is within 3 bits
    y = MASK_N_BITS(y, 3); // Ensure y is within 3 bits
    colorCode = MASK_N_BITS(colorCode, 4);
    return (opcode << 28) | (tile_id << 23) | (colorCode << 19) | (x << 16) | (y << 13);
}
 
int cmdGenSetActPixTile(int tile_id, int colorCode, int x, int y) {
    int opcode = 0x7;
    tile_id = MASK_N_BITS(tile_id, 4); // Ensure tile_id is within 4 bits
    x = MASK_N_BITS(x, 4); // Ensure x is within 3 bits
    y = MASK_N_BITS(y, 4); // Ensure y is within 3 bits
    colorCode = MASK_N_BITS(colorCode, 4);
    return (opcode << 28) | (tile_id << 24) | (colorCode << 20) | (x << 16) | (y << 12);
}
